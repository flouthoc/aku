...
int TradeManager::performBuyInternal(Trade trade){
	...
	if (this->canBuy(trade)){
		...
		if (DEBUG_FLAG){
			std::string trade_log = "Bought - "+std::to_string(trade.quantity)+" at "+std::to_string(trade.tick.close)+" on "+std::to_string(trade.tick.time);
		}
...
int TradeManager::performSellInternal(Trade trade){
	...
	if (this->canSell(trade)){
		...
		if (DEBUG_FLAG){
			std::string trade_log = "Sold - "+std::to_string(trade.quantity)+" at "+std::to_string(trade.tick.close)+" on "+std::to_string(trade.tick.time);
		}
